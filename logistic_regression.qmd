...
Finally, the final dataset is obtained, which will be used from now on, assesing different logistic regression models.
...
First, it is important to assess if the response variable is imbalance.
As it is show below, `cardio` has 2 classes which are balanced.
